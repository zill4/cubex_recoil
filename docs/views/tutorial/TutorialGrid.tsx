...
import { Align, Button, Toolbar, Grid, Checkbox, Table, Layer, Dropdown, Input, Wizard, Modal, Open, Emerge, SlideIn, Transform, Toggle, Shrink } from '../../../src/index';
...
export default class TutorialTable extends React.Component<any, any>{
  ...
  render() {
    ...
    let colTemplate = (item, index) =>{
      return <Layer theme="light" fill border flexCenter>{item.name}</Layer>
    }

    let example = () => {
      return (
        <div>
          <Grid margin="20px" className="mb20" rows={[{ columns: [1, 1, 1, 1, 1], height:'200px', template: colTemplate.bind(this) }]} dataSource={TableProperties} />

          <Grid className="mb20" margin={"20px"} rows={[
          ...
          <Grid margin="20px" rows={[{ columns: [1, 1, 1, 1, 1], height:'200px', template: colTemplate.bind(this) }]} dataSource={TableProperties} />
        </div>
      )
    }
